...
export class PackageData {
    ...
    public get query_url(): string {
        ...
        if (queryUrl === null || queryUrl === undefined || queryUrl === "") {
            queryUrl = `http://localhost:8080/config-service/api/config/file?path=config/${spid}-query-h02.json`;
        }
        const sep = queryUrl.includes('?') ? '&' : '?';
        return `${queryUrl}${sep}plat=${spid}&version=${version}&pkg=${pkg}&device=${device}`
        ...
        for (let key in data) {
            if (key == "isDebug" || key == "isOut") {
                if (typeof data[key] == "string") {
                    (this.g_UserInfo as any)[key] = data[key] == "true";
                } else {
            ...
            if (key == "spid") {
                ...
                if (this.g_UserInfo[key] && this.g_UserInfo[key] != undefined && this.g_UserInfo[key] != "undefined" && data[key] == "chrome") {
                    // skip — keep existing non-chrome device id
                } else {
                    (this.g_UserInfo as any)[key] = data[key];
                }
            } else {
    ...
    public setQueryData(data: url_parm) {
        ...
        if (data.server_info.account_spid) {
            this.g_UserInfo.account_spid = data.server_info.account_spid;
        }
    }
...
class packageInfo {
    // index signature so dynamic key access compiles cleanly
    [key: string]: any;
...
type param_List = {
    ...
    notice_query_url: string;          // thêm — có trong JSON thực
    error_report_url: string;          // thêm — có trong JSON thực
...
type switch_List = {
    ...
    game_moniter?: boolean;            // thêm — có trong JSON thực
}
...
export type server_Info = {
    ...
    server_time: number;
    spid: string;
    account_spid: string;              // thêm — có trong JSON thực
    server_offset: number;             // thêm — có trong JSON thực
}
...
type version_Info = {
    assets_info: {
        ...
        resources: string;
        res: string;
        main: string;                  // thêm — có trong JSON thực
    };
