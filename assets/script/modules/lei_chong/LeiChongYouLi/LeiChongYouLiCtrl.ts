...
export class LeiChongYouLiCtrl extends BaseCtrl {
    MsgCfg(): regMsg[] {
        return [
    ...
    protected initCtrl() {
        ...
        this.handleCollector.Add(SMDHandle.Create(RechargeData.Inst().ResultData, LeiChongYouLiData.Inst().FlushSMD.bind(LeiChongYouLiData.Inst()), "is_change"));
        this.handleCollector.Add(RemindRegister.Create(Mod.LeiChong.LeiChongYouLi, LeiChongYouLiData.Inst().ResultData, LeiChongYouLiData.Inst().GetRed.bind(LeiChongYouLiData.Inst())));
    }

    private recvSCLeiChongInfo(data: PB_SCRaLeiChongInfo) {
        LeiChongYouLiData.Inst().SetLeiChongInfo(data);
    }
...
export class LeiChongYouLiData extends DataBase {
    ...
    **/
    ...
    public IsLeiChongActShow() {
        ...
        if (!this.result_data.info) {
            LeiChongYouLiCtrl.Inst().SendReq(LeiChong_OP_TYPE.INFO);
            return false;
        }
        let flag = DataHelper.ToBinary(this.result_data.info.fetchFlag);
        let cfg = CfgLeiChong.reward;
