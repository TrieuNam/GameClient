...
export class BoxFundData extends DataBase {
    ...
    public GetMaxPause() {
        if (CfgBoxFundData.other && CfgBoxFundData.other.length > 0 && CfgBoxFundData.other[0]) {
            return CfgBoxFundData.other[0].phase || 0
        }
        if (!CfgBoxFundData.phase_configure || CfgBoxFundData.phase_configure.length == 0) {
            return 0
        }
        let max_phase = 0
        for (const cfg of CfgBoxFundData.phase_configure) {
