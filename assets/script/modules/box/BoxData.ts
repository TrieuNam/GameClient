...
export class BoxResultData {
    ...
    @smartdata
    compare_info: PB_SCBoxEquipCompareInfo;
...
export class BoxData extends DataBase {
    ...
    private GetBoxOtherCfg() {
        let other = CfgBoxData ? (CfgBoxData as any).other : null;
    ...
    public GetBoxNumInfo() {
        let boxId = this.GetBoxId();
        if (boxId <= 0) {
            return 0;
        }
        let num = Item.GetNum(boxId);
        return num;
    }
    ...
    public GetBoxCompareInfo() {
        return this.box_result_data.compare_info;
    }

    public GetBoxEquippedBefore() {
        const compareInfo = this.box_result_data.compare_info as any;
        const equippedBefore = this.readCompareEquip(compareInfo, false);
        if (equippedBefore && equippedBefore.itemId > 0) {
            return equippedBefore;
        }
        return null;
    }
    public GetBoxStatus() {
    ...
    public setBoxCompareEquip(data: PB_SCBoxEquipCompareInfo) {
        const raw = data as any;
        const candidate = this.readCompareEquip(raw, true);
        const equippedBefore = this.readCompareEquip(raw, false);

        if (raw) {
            // Normalize both naming styles for downstream reads.
            raw.candidateEquip = candidate;
            raw.equippedBefore = equippedBefore;
        }

        this.box_result_data.compare_info = raw as PB_SCBoxEquipCompareInfo;

        let mapped = new PB_SCBoxEquipInfo();
        if (candidate) {
            mapped.isNew = this.readCompareIsNew(raw, candidate);
            mapped.equipInfo = this.cloneEquipData(candidate);
        } else {
            mapped.isNew = 0;
            mapped.equipInfo = new PB_EquipData();
            mapped.equipInfo.equipType = -1;
            mapped.equipInfo.itemId = 0;
        }

        this.setBoxEquip(mapped);
    }

    private readCompareEquip(compareInfo: any, isCandidate: boolean): PB_EquipData | null {
        if (!compareInfo) {
            return null;
        }

        const direct = isCandidate
            ? (compareInfo.candidateEquip ?? compareInfo.candidate_equip)
            : (compareInfo.equippedBefore ?? compareInfo.equipped_before);
        if (direct && direct.itemId > 0) {
            return this.cloneEquipData(direct);
        }
        return null;
    }

    private readCompareIsNew(compareInfo: any, candidate: PB_EquipData): number {
        if (compareInfo && typeof compareInfo.isNew === "number") {
            return compareInfo.isNew;
        }
        if (compareInfo && typeof compareInfo.is_new === "number") {
            return compareInfo.is_new;
        }
        return candidate && candidate.equipType == -1 ? 0 : 1;
    }

    private cloneEquipData(src: any): PB_EquipData {
        const out = new PB_EquipData();
        if (!src) {
            return out;
        }
        out.equipType = src.equipType ?? src.equip_type ?? -1;
        out.itemId = src.itemId ?? src.item_id ?? 0;
        out.hp = src.hp ?? 0;
        out.attack = src.attack ?? 0;
        out.defend = src.defend ?? src.defense ?? 0;
        out.speed = src.speed ?? 0;
        out.attrType1 = src.attrType1 ?? src.attr_type1 ?? 0;
        out.attrValue1 = src.attrValue1 ?? src.attr_value1 ?? 0;
        out.attrType2 = src.attrType2 ?? src.attr_type2 ?? 0;
        out.attrValue2 = src.attrValue2 ?? src.attr_value2 ?? 0;
        return out;
    }

    public setBoxLevel(data: PB_SCBoxInfo) {
        let max_arena = this.GetArenaMax();
        if (max_arena > 0 && this.box_result_data.box_level_data && (this.box_result_data.box_level_data.arenaItemNum == max_arena - 1) && data.arenaItemNum == max_arena) {
    ...
    public GetBoxLevel() {
        ...
        return Math.max(level, 1); // boxLevel=0 là default protobuf khi player mới → clamp về 1
    }
    ...
    public GetBoxRandomColorCfg(lv?: number) {
        ...
        lv = Math.max(1, Math.min(lv, cfgs.length)); // clamp [1, max]
        let cfg = cfgs[lv - 1];
        return cfg;
    }
    ...
    public GetBuyBoxCurrId() {
        let other = this.GetBoxOtherCfg();
        return other.currency_type || 0;
    }

    public GetBoxSpeedItemId() {
        let other = this.GetBoxOtherCfg();
        return other.accelerate_id || 0;
    }
    ...
    public GetBoxId() {
        let other = this.GetBoxOtherCfg();
        return other.unpack_item_id || 0;
    }

    public IsBoxCanUp() {
        let self = this
        let curcfg = self.GetBoxRandomColorCfg();
        if (!curcfg) return false;
