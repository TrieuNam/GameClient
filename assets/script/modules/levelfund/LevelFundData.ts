...
export class LevelFundData extends DataBase {
    ...
    public GetMaxPause() {
        if (CfgLevelFundData.other && CfgLevelFundData.other.length > 0 && CfgLevelFundData.other[0]) {
            return CfgLevelFundData.other[0].phase || 0
        }
        if (!CfgLevelFundData.phase_configure || CfgLevelFundData.phase_configure.length == 0) {
            return 0
        }
        let max_phase = 0
        for (const cfg of CfgLevelFundData.phase_configure) {
            if (cfg.phase > max_phase) {
                max_phase = cfg.phase
            }
        }
        return max_phase
