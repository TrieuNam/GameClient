...
export class NetManager extends Singleton {
    ...
    public ConnectServer(host: string, port: number, callBack: (suc:boolean,opt:NetConnectOptions)=>void, autoReconnect:number = 10, token?: string): void {
        let self = this;
        const _host = host;
        const _port = port;
        self._netNode.Connect({ host: _host, port: _port, autoReconnect: autoReconnect, token: token }, callBack)
    }
