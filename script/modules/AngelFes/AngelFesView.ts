...
import { AngelFesCtrl, AngelFesData } from "./AngelFesCtrl";

@BaseView.registView
export class AngelFesView extends BaseView {
    select_index = 0;
    private time_timer: any;
    private realtime = 0
    private cache_timer = 0
    tabbarCfg: tabberInfo[] = [
        { panel: null, viewName: "", titleName: Language.AngelFes.TagName[0], index: 0, modKey: null, isRemind: false },
        { panel: null, viewName: "", titleName: Language.AngelFes.TagName[1], index: 1, modKey: null, isRemind: false },
        // { panel: null, viewName: "", titleName: Language.AngelFes.TagName[2], index: 2, modKey: null, isRemind: false },
    ]
    protected viewRegcfg: viewRegcfg = {
        UIPackName: "AngelFes",
        ViewName: "AngelFesView",
    ...
    protected extendsCfg = [
        { ResName: "TaskItem", ExtendsClass: AngelFesTaskItem },
        { ResName: "ShopItem", ExtendsClass: AngelFesShopItem },
        { ResName: "BtnBuy", ExtendsClass: AngelFesShopBtnBuy },
    ]
    InitData() {
        this.viewNode.Board.SetData(new BoardData(AngelFesView, Language.AngelFes.Title))
        this.viewNode.TagList.SetData(this.tabbarCfg);
        ...
        this.AddSmartDataCare(AngelFesData.Inst().flush_info, this.flushInfoPanel.bind(this), "flush_need");
    ...
    public flushInfoPanel() {
        if (this.select_index == 0) {
            let list = AngelFesData.Inst().GetTask0List()
            this.viewNode.TaskList.SetData(list)
        }
        else if (this.select_index == 1) {
            let list = AngelFesData.Inst().GetTask1List()
            this.viewNode.TaskList.SetData(list)
        }
        else if (this.select_index == 2) {
            let list = AngelFesData.Inst().GetShopList()
            this.viewNode.ShopList.SetData(list)
        }

        let view_param = AngelFesData.Inst().GetViewParam()
        this.cache_timer = view_param.timer

        this.viewNode.RedPoint0.SetNum(AngelFesData.Inst().GetRed0Num())
        this.viewNode.RedPoint1.SetNum(AngelFesData.Inst().GetRed1Num())
        this.viewNode.RedPoint2.SetNum(AngelFesData.Inst().GetFreeGiftRed())
        this.FlushFlushTime()
    }
    ...
    public ClickShopItem(item: AngelFesShopItem) {
        if (item.data.limit_full) {
            PublicPopupCtrl.Inst().Center(Language.AngelFes.LimitBuyFull)
            return
        }
        ...
        if (item.data.price_type == 3) {
            ...
            let order_data = Order_Data.initOrder(seq, ACTIVITY_TYPE.FaZhenGala, money / 10, money, "");
            OrderCtrl.generateOrder(order_data);
        }
        else {
            if (item.data.price_num > 0) {
                AngelFesData.Inst().TryBuy({
                    seq: item.data.seq,
                ...
                AngelFesCtrl.Inst().SendAngelFesReq(2, item.data.seq)
            }
...
export class AngelFesTaskItem extends fgui.GComponent {
    ...
    public OnClickOper() {
        if (!this.data.is_complete) {
            PublicPopupCtrl.Inst().Center(Language.AngelFes.TaskUnComplete)
            return
        }

        AngelFesCtrl.Inst().SendAngelFesReq(1, this.data.task_id)
    }
}

export class AngelFesShopItem extends fgui.GComponent {
    private viewNode: { [key: string]: any } = {
        ItemCell: <ItemCell>null,
        ...
        BtnBuy: <AngelFesShopBtnBuy>null,
    ...
    public SetData(data: any) {
        ...
        if (data.price_type == 4) {
            if (!data.limit_full) {
                this.viewNode.redPoint.SetNum(AngelFesData.Inst().GetFreeGiftRed());
            }
        } else {
        ...
        // {
        //     UH.SpriteName(this.viewNode.BtnBuy,"AngelFes","RenMinBi");    
        // }
        ...
        this.viewNode.BtnFree.grayed = data.limit_full
    ...
    public OnClickOper() {
        ...
        if (this.data.price_type == 3) {
            ...
            if (this.data.price_num > 0) {
                ...
                AngelFesCtrl.Inst().SendAngelFesReq(2, this.data.seq)
            }
...
export class AngelFesShopBtnBuy extends fgui.GButton {
    private viewNode = {
...
export class AngelBtnFes extends fgui.GButton {
    ...
    protected onConstruct() {
        ...
        this.viewNode.timer.StampTime(AngelFesData.Inst().GetEndTime(),
            TimeFormatType.TYPE_TIME_4)

        this.handleCollector = HandleCollector.Create();

        var handle = SMDHandle.Create(AngelFesData.Inst().flush_info, this.FlushRedPoint.bind(this), "flush_need");
    ...
    private FlushRedPoint() {
        if (this.viewNode.RedPoint != undefined && ViewManager.Inst().IsOpen(AngelView)) {
            this.viewNode.RedPoint.SetNum(AngelFesData.Inst().GetRedNum())
        }
    }
