import { CfgStarMapFesData } from "config/CfgStarMapFes";
...
export class StarMapFesCtrl extends BaseCtrl {
    MsgCfg(): regMsg[] {
        return [
            { msgType: PB_SCRaStarMapGalaInfo, func: this.onSCStarMapGalaInfo },
            //         { msgType: PB_SCRuneRet, func: this.onSCRuneRet },
        ]
    }

    private onSCStarMapGalaInfo(protocol: PB_SCRaStarMapGalaInfo) {
        LogError("3025?星图盛典下发?2063", protocol)
        StarMapFesData.Inst().SetStarMapFesInfo(protocol)
    }

    public SendStarMapFesReq(operaType: number, param1: number) {
        LogError("3000?星图盛典购买?2066", operaType, param1)
        ActivityCtrl.Inst().SendAngelReq(ACTIVITY_TYPE.StarMapGala, operaType, param1, 0, 0)
    }

    protected initCtrl() {
        this.handleCollector.Add(RemindRegister.Create(Mod.StarMap.View,
            StarMapFesData.Inst().flush_info,
            StarMapFesData.Inst().GetRedNum.bind(StarMapFesData.Inst())));
    }
}

class StarMapFesInfo {
    @smartdata
    flush_need: number;
    @smartdata
    show_time: number;
...
export class StarMapFesData extends DataBase {
    public flush_info: StarMapFesInfo;
    ...
    private createSmartData() {
        this.flush_info = CreateSMD(StarMapFesInfo);
        this.flush_info.flush_need = 0
        this.flush_info.show_time = 0
    }

    public SetStarMapFesInfo(protocol: PB_SCRaFaZhenGalaInfo) {
    ...
    public GetTask0List() {
        ...
        let list = []
        for (var index in CfgStarMapFesData.task_configuration) {
            let cfg = CfgStarMapFesData.task_configuration[index]
            if (cfg.start_level <= role_level && cfg.end_level >= role_level) {
                let oper = cfg
                if (oper.task_show == 1) {
                    let num = this.GetTaskParam(oper.task_type)
                    ...
                    let info = {
                        ...
                        btn_name: is_complete ? (is_got ? Language.StarMapFes.BtnDone : Language.StarMapFes.BtnComplete) : Language.StarMapFes.BtnUnComplete,
    ...
    public GetShopList() {
        ...
        let list = []
        for (var index in CfgStarMapFesData.gift_configuration) {
            let cfg = CfgStarMapFesData.gift_configuration[index]
            if (cfg.start_level <= role_level && cfg.end_level >= role_level) {
                let oper = cfg
                let buy_time = this.GetBuyGiftTime(oper.type)
                let limit_str = TextHelper.Format(Language.StarMapFes.GiftLimit[oper.limit_type], (buy_time + "/" + oper.limit_convert_count))
                ...
                let info = {
                    ...
                    off_str: oper.discount + Language.StarMapFes.Offset,
    ...
    public GetRedNum() {
        if (!ActivityRandData.Inst().IsACtOpen(ACTIVITY_TYPE.FaZhenGala)
            || !FunOpen.Inst().GetFunIsOpen(Mod.StarMapFes.Main).is_open || StarMapFesData.Inst().flush_info.show_time == 0) {
            return 0
        }
    ...
    public TryBuy(param: any) {
        let buy_fun: Function = () => {
            StarMapFesCtrl.Inst().SendStarMapFesReq(2, param.seq)
        };
    ...
    public FixTagList(list: any) {
        let check_list = [this.GetTask0List(), this.GetTask1List(), this.GetShopList()]

        let re_list = []
        for (var index in check_list) {
            if (list[index] && check_list[index].length > 0) {
                re_list.push(list[index])
            }
        }
