...
export class Main extends Singleton {
    ...
    public MainLogin(cb: (uInfo: LoginVerify) => void) {
        ...
        HTTP.GetJson(url, (status, resp, text) => {
            this.LoginRet(cb, status, resp, text);
        });
    }
    private timer_handle: any;
    private timeOut = 0;
    private LoginRet(cb: (uInfo: LoginVerify) => void, statusCode: number, data: LoginVerify) {
        ...
        if (this.isConnect) {
            ...
            if (!succ) {
                ...
                if (LoginData.Inst().ResultData.result == LoginAckResult.LOGIN_RESULT_SUC) {
                    ChannelAgent.Inst().wxModal("提示", "连接服务器失败，请检查网络状态后重试", () => {
                        ChannelAgent.Inst().ExitProgram();
                        // let MainAgent = (window as any)['MainAgent'];
                        // MainAgent.exitMiniProgram();
                    }, null, false);
                } else {
                    PublicPopupCtrl.Inst().Center(`连接失败`);
                }
            }
        }
    }
